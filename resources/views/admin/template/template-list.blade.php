			...
			<div class="col-sm-12">
				<div class="card">
					<div class="card-header">
						<a href="{{url('auth/add-email-template')}}" class="btn btn-info">Add Template</a>
					</div>
					<div class="card-body">
							...
							<table class="table table-bordered">
								...
								<tbody>
									...
									<tr row_id="{{$row->id}}" class="get_row_{{$row->id}}">
										<td>{{$key+1+($templates->currentPage()-1) * ($templates->perPage())}}</td>
										<td>{{$row->title}}</td>
										<td>{{$row->created_at}}</td>
										<td>@if($row->status==1)Active @else Inactive @endif</td>
										<td>
											<div class="actions">
												<a class="btn btn-sm bg-success-light" href="{{url('auth/edit-email-template/'.$row->id)}}">
