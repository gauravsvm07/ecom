			...
			<div class="col-sm-12">
				<div class="card">
					<div class="card-header">
						<a href="{{url('auth/add-page-section')}}" class="btn btn-info">Add Section</a>
					</div>
					<div class="card-body">
						...
						<div class="table-responsive">
							<table class="table table-bordered">
								...
								<tbody>
									...
									@foreach($page_section as $key=>$row)
									<tr row_id="{{$row->id}}" class="get_row_{{$row->id}}">
										<td>{{$key+1+($page_section->currentPage()-1) * ($page_section->perPage())}}</td>
										<td>{{$row->name}}</td>
										<td>@if($row->status==1) Active @else Inactive @endif</td>
										<td>
											<div class="actions">
												<a class="btn btn-sm bg-success-light" href="{{url('auth/edit-page-section/'.$row->id)}}">
...
<script>
	$(document).ready(function() {
		...
		$('.deleteBtnYs').on('click', function() {
			...
			$.ajax({
				url: "{{url('auth/delete-page-section')}}",
				method: "post",
				data: {
					id: row_id,
					_token: csrf_token
